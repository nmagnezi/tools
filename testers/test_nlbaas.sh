...
function wait_for_loadbalancer_active() {
  lb_name=$1
  while [ $(neutron lbaas-loadbalancer-show $lb_name -f value -c provisioning_status) != "ACTIVE" ]; do
    sleep 2
  done
}
...
neutron lbaas-listener-create --loadbalancer $LB_NAME --protocol HTTP --protocol-port 80 --name $LISTENER_NAME
wait_for_loadbalancer_active $LB_NAME
neutron lbaas-pool-create --lb-algorithm ROUND_ROBIN --listener $LISTENER_NAME --protocol HTTP --name $POOL_NAME
wait_for_loadbalancer_active $LB_NAME
neutron lbaas-member-create  --subnet $SUBNET_ID --address $IP1 --protocol-port 80 $POOL_NAME
wait_for_loadbalancer_active $LB_NAME
neutron lbaas-member-create  --subnet $SUBNET_ID --address $IP2 --protocol-port 80 $POOL_NAME


echo "How to test load balancing:"
echo ""
echo "${NAMESPACE_CMD_PREFIX} ${NAMESPACE_NAME} curl $(neutron lbaas-loadbalancer-show ${LB_NAME} -f value -c vip_address)"
echo ""
