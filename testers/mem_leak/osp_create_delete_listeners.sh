...
do
  ...
  do
    neutron lbaas-listener-create --loadbalancer $LB_NAME --protocol HTTP --protocol-port $j --name listener_$j -c name --format value >> mem_usage.txt
    sleep 2
  done
  for k in `seq 1 $LISTENERS_AMOUNT`
  do
    neutron lbaas-listener-delete $i >> mem_usage_create_delete
    sleep 2
  done
